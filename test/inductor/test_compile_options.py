...
class TestCompileOptions(TestCase):
    ...
    def test_patch_inductor_backend_swapped_config_drops_options(self):
        # the option->key mapping only holds for the original config module;
        # swapping it must not validate the old keys against the new module
        with fake_backend_routes():
            with patch_inductor_backend(
                "fake_device_a", custom_backend_config=fake_config_module2
            ):
                self.assertIsNone(get_compile_option_owner("fake_backend_bool"))
            # passing the original module back keeps the options
            with patch_inductor_backend(
                "fake_device_a", custom_backend_config=fake_config_module
            ):
                owner = get_compile_option_owner("fake_backend_bool")
                self.assertEqual(owner, (fake_config_module, "e_bool"))
            owner = get_compile_option_owner("fake_backend_bool")
            self.assertEqual(owner, (fake_config_module, "e_bool"))
    ...
    def test_apply_options_unknown_name_rejected(self):
        with fake_backend_routes():
            with self.assertRaisesRegex(
                RuntimeError, "Unexpected optimization option unregistered_option"
            ) as ctx:
                torch._TorchCompileInductorWrapper(
                    None, {"unregistered_option": True}, None
                )
            # the known-option list includes backend-declared names
            self.assertIn("fake_backend_bool", str(ctx.exception))
    ...
    def test_list_options_does_not_register_backends(self):
        # list_options() must not trigger deferred backend registration
