...
def register_backend_for_device(
    ...
    if device_compile_options:
        ...
        for name, key in device_compile_options.items():
            ...
            for other_device, other in list(device_codegens.items()):
...
def list_compile_options() -> list[str]:
    ...
    A pure query over current registrations; backends that register lazily
    appear once they have registered.
    """
    names: list[str] = []
    for codegen in list(device_codegens.values()):
...
def patch_compile_options(config_patches: dict[str, Any] | None):
    ...
    for name, value in (config_patches or {}).items():
        normalized = name.replace("-", "_")
        if normalized not in config._config:  # type: ignore[attr-defined]
            # a deferred privateuse1 backend may not have registered yet
            init_backend_registration()
        owner, key = get_compile_option_owner(normalized) or (core_config, normalized)
