...
class _TorchCompileInductorWrapper:
    ...
    def apply_options(self, options: dict[str, _Any] | None) -> None:
        ...
        for key, val in options.items():
            ...
            if attr_name not in config._config:  # type: ignore[attr-defined]
            ...
            if target_key not in owner_config._config:  # type: ignore[attr-defined]
                known = list(config.get_config_copy()) + list_compile_options()
                raise RuntimeError(
                    f"Unexpected optimization option {key}, known options are {known}"
                )
